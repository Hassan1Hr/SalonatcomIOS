...
extension BillViewController : UITableViewDelegate , UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if UIScreen.main.nativeBounds.height == 2688 {
            if indexPath.section == 1 {
                return UIScreen.main.nativeBounds.height * 0.060
            }
            return UIScreen.main.nativeBounds.height * 0.040
        }else{
            if indexPath.section == 1 {
                return UIScreen.main.nativeBounds.height * 0.080
            }
            return UIScreen.main.nativeBounds.height * 0.050
        }
    }
    ...
    func tableView(_ tableView: UITableView, willDisplayHeaderView view: UIView, forSection section: Int) {
        ...
        if deviceType == "iPad" {
            FixHeight().FontSizeText(Text:  header.textLabel!, Size: 30)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if indexPath.section == 1 {
           ...
           // image data   >>>> CollectionData[0].services[indexPath.row].image
            ...
            cell.cells.salary.textColor = .red
            ...
            let deviceType = UIDevice.current.deviceText
            if deviceType == "iPad" {
                   FixHeight().FontSizeText(Text:  cell.cells.name, Size: 23)
                   FixHeight().FontSizeText(Text:  cell.cells.salary , Size: 20)
                   FixHeight().FontSizeText(Text:  cell.cells.time , Size: 20)
            }else if UIScreen.main.nativeBounds.height >= 2436 {
                cell.cells.salary.font = UIFont.preferredFont(forTextStyle: .caption1)
                cell.cells.time.font = UIFont.preferredFont(forTextStyle: .caption1)
                cell.cells.name.font = UIFont.boldSystemFont(ofSize: 16)
            }else{
                cell.cells.salary.font = UIFont.preferredFont(forTextStyle: .caption2)
                cell.cells.time.font = UIFont.preferredFont(forTextStyle: .caption2)
                cell.cells.name.font = UIFont.boldSystemFont(ofSize: 14)
            }
            return cell
        }
        else{
            ...
            let deviceType = UIDevice.current.deviceText
            if deviceType == "iPad" {
                FixHeight().FontSizeText(Text:  cell.info, Size: 22)
                FixHeight().FontSizeText(Text:  cell.label , Size: 25)
            }else if UIScreen.main.nativeBounds.height >= 2436 {
                cell.label.font = UIFont.preferredFont(forTextStyle: .body)
                cell.info.font = UIFont.boldSystemFont(ofSize: 16)
            }else{
                cell.label.font = UIFont.preferredFont(forTextStyle: .caption2)
...
class infoRequst : UITableViewCell{
    ...
    fileprivate func setupLabel(stack : UIStackView , label : UILabel){
        ...
        label.widthAnchor.constraint(equalTo: stack.widthAnchor, multiplier: 0.25).isActive = true
